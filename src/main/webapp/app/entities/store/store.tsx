...
export const Store = (props: IStoreProps) => {
  ...
  const { storeList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="store-heading">
        Stores
          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('move')}>
                  Move <FontAwesomeIcon icon="sort" />
                </th>
                <th>
            ...
            <tbody>
              {storeList.map((store, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{store.date ? <TextFormat type="date" value={store.date} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{store.quantity}</td>
                  <td>{store.move}</td>
                  <td>{store.productId ? <Link to={`product/${store.productId}`}>{store.productId}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${store.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${store.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
